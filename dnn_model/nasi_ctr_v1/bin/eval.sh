...
python3 ${code_dir}/main.py --model_dir "${model_dir}/${ckpt}/join" --mode eval --type "join" \
    ...
    --time_str "${time_str}" --time_format "${time_format}" > ${model_dir}/logs/${ckpt}.eval 2>&1
rm -rf ${model_dir}/${ckpt}/join/eval

msg=$(grep 'INFO:tensorflow:Saving dict for global step' ${model_dir}/logs/${ckpt}.eval|awk -F': ' '{print $2}')
echo "task = $(basename ${code_dir}), time = ${ckpt}, ${msg}" >> ${model_dir}/logs/eval
