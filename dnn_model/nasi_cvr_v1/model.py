...
class DnnModel:
    def __init__(self, emb_map, features, slot_list, is_training=True):
        ...
        self.probs = [1 / (w / p - w + 1) for p in self.probs]
        ...
        for i, (label, logit) in enumerate(zip(self.labels, self.logits), start=1):
            n = tf.reduce_sum(tf.ones_like(label, dtype=tf.float32)) + 1e-20
            prob = tf.sigmoid(logit)
            
            loss = -tf.reduce_sum(w * label * tf.math.log(prob + 1e-20) + (1 - label) * tf.math.log(1 - prob + 1e-20)) / n
            self.loss += loss
